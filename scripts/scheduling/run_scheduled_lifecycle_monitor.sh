#!/usr/bin/env bash
# Scheduled lifecycle monitor (Scheduler B) — launchd at 17:00 IST once daily (OHM Phase 1 template).
set -euo pipefail
...
LOG_FILE="$LOG_DIR/lifecycle-monitor-${STAMP}.log"
...
echo "============================================================"
echo "Scheduled lifecycle monitor started: $(date -Iseconds)"
echo "ROOT=$ROOT"
...
export LIFECYCLE_MONITOR_RUN_TRIGGER=scheduled

PAUSE_MSG="SKIP: lifecycle monitor paused by operator ($(date -Iseconds))"
if ! "$PYTHON" scripts/scheduling/check_operator_pause.py \
  --scheduler lifecycle \
  --skip-message "$PAUSE_MSG"; then
  echo "$PAUSE_MSG"
  echo "============================================================"
  echo "Scheduled lifecycle monitor skipped (operator pause): $(date -Iseconds)"
  echo "============================================================"
  exit 0
fi
...
if ! "$PYTHON" scripts/scheduling/probe_file_lock.py \
  ...
  echo "$ACQ_SKIP_MSG"
  echo "============================================================"
  echo "Scheduled lifecycle monitor skipped (acquisition lock held): $(date -Iseconds)"
  echo "============================================================"
  exit 0
fi
